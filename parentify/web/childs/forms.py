...
class FormChild(FormBase):
    ...
    def __init__(self, request, child_id=None):
        ...
        if request.current_user and request.current_user.is_admin:
            ...
            self.fields['user_id'].choices = choise_name_orm(request, User, True, ['first_name','last_name']) if request.current_user.is_admin and not request.path.startswith('/profile/') else [(request.current_user.id,request.current_user.full_name)]
